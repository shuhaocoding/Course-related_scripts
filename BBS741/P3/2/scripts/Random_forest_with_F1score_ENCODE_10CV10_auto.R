...
setwd("~/Desktop/FOXA1_motif_freq/");
...
foreach(h = 1:length(dirs)) %dopar% {
  ...
  folder<-paste0("~/Desktop/FOXA1_motif_freq/",dirs[h])
  print(folder)
  setwd(folder)
  dd<-na.omit(read.table("tmp4.out"))[-1,-1]
motifName <- na.omit(read.table("tmp4.out")[-1,1])
#dd[,1:3000]->dd
...
label<-as.factor(c(rep(1,500),rep(2,500),rep(3,500),rep(4,500)) )
...
modelpredic <- function (z){
  ...
  print(results)
  results[1,1]+results[2,2]+results[3,3]+results[4,4] ->correct
  ...
  #sapply(1:4,F1score) -> F1scores
  F1scores<-NULL
  for(j in 1:4){F1score(j)->F1scores[j]}
