...
library(foreach)
library(doMC)
registerDoMC(4)  # Change 6 to your CPU cores number.
...
save(RFmodel_1000, file = "RFmodel_120_tree1k_FOXA1.Rdata")

plot(RFmodel_1000$err.rate[,1], log="y",col=1, type = "l", xlab="Number of trees", ylab = "Error"
     ,main = "Out of Bag (OOB) evaluation of RF of FOXA1 dataset")
...
fname <- paste0(j,"_AUC_FOXA1.Rdata")
save(AUC, file = fname)
}
...
list.files(pattern = "*_AUC_FOXA1.Rdata")->Rfiles
...
pdf("AUC_TCF_100times_FOXA1.pdf",width = 4,height = 2)

dodge <- position_dodge(width = 0.9)
limits <- aes(ymax = data$AUROC + data$sd,
              ymin = data$AUROC - data$sd)

data$Cells<-c("HEK293T", "HepG2", "K562", "MCF-7")
